--! Previous: sha1:c62022657c56a0f5d042fa396ad53dc56e94aad2
--! Hash: sha1:b48d09b35bad3331936eb369c8bd8e4eed96ce7c

-- Perf fix: Postgres 12+ auto-inlines a CTE referenced exactly once, which
-- duplicated the start/end rating subqueries across the SELECT list, the
-- null-check filter, and the ORDER BY (4x redundant work, 173k candidates ->
-- ~14s). "materialized" forces the CTE to compute once per candidate.
...
as $$
    ...
    changes as materialized (
        select
            c.fideid, c.name, c.country, c.title,
            (
    ...
    select fideid, name, country, title, start_rating, end_rating, end_rating - start_rating as delta
    from changes
