--! Previous: sha1:4a35cb3cfd7f0e8b1394be56e76feeeb929ffb90
--! Hash: sha1:7c4b528ccfa349bc7d37d7f36d47886474b9faf7

-- FIDE federation names/flags, sourced from FIDE's own dropdown
-- (ratings.fide.com/advanced_search.phtml), cross-checked against the
-- distinct codes actually present in `ratings`. iso2 is null for
-- non-ISO/defunct federations (ENG, SCO, WLS, FID, AHO, ...) -- the
-- frontend falls back to the plain name and skips the flag for those.
-- Moved from a static frontend data file into the DB so it's one source
-- of truth alongside the `countries` table the scraper already maintains.
...
insert into countries (code, name, iso2) values
    ...
    ('YEM', 'Yemen', 'YE'),
    ('ZAM', 'Zambia', 'ZM'),
    ('ZIM', 'Zimbabwe', 'ZW')on conflict (code) do update set name = excluded.name, iso2 = excluded.iso2;
